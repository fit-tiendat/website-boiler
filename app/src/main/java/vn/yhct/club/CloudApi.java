...
public class CloudApi {
    ...
    private String request(String method,String path,String body,boolean auth) throws Exception {
        ...
        c.setRequestProperty("apikey",KEY); c.setRequestProperty("Content-Type","application/json"); c.setRequestProperty("Accept","application/json");
    ...
    public JSONArray profiles(String q) throws Exception { String p="/rest/v1/profiles?select=id,display_name,avatar_url,bio,member_code,role,position,department,total_points&order=display_name.asc&limit=100"; if(q!=null&&!q.trim().isEmpty())p+="&display_name=ilike.*"+enc(q.trim())+"*"; return new JSONArray(request("GET",p,null,true)); }
